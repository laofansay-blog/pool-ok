...
export const gameAPI = {
	...
	async placeBet(
		...
		try {
			const response = await fetch(
				`${supabase.supabaseUrl}/functions/v1/place-bet`,
				{
					method: 'POST',
					headers: {
	...
	async getBetHistory(
		userId: string,
		limit: number = 20
	): Promise<{ data: BetHistory[]; error: any }> {
		try {
			const response = await fetch(
				`${supabase.supabaseUrl}/functions/v1/get-history`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
...
export const utils = {
	...
	validateSelectedNumbers(numbers: number[]): boolean {
		...
		return numbers.every(
			(num) =>
				num >= GAME_CONFIG.NUMBER_RANGE[0] && num <= GAME_CONFIG.NUMBER_RANGE[1]
		)
	},
