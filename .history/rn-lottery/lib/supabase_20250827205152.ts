...
export const placeBet = async (betRequest: BetRequest) => {
	try {
		...
		// 临时解决方案：模拟成功的投注，直到Edge Function部署成功
		console.log('使用模拟投注模式...')
		...
		const mockResponse = {
			...
			data: {
				betId: `bet_${Date.now()}`,
				userId: 'test-user-id',
				totalAmount: betRequest.totalAmount,
				potentialPayout: betRequest.potentialPayout,
				betsCount: betRequest.bets.length,
				timestamp: new Date().toISOString()
			},
			message: '投注成功（模拟模式）'
		}

		console.log('模拟投注成功:', mockResponse)
		return mockResponse
