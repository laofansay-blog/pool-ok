...
export default function WinningsScreen() {
	...
	const loadWinnings = async () => {
		try {
			...
			const formattedWinnings: WinningRecord[] = (data || []).map(
				(bet: any) => ({
					id: bet.id,
			...
			// 计算总中奖金额和总盈利
			const totalWin = formattedWinnings.reduce(
				(sum, win) => sum + win.actual_payout,
				0
			)
			const totalProf = formattedWinnings.reduce(
				(sum, win) => sum + win.profit,
				0
			)
	...
	const renderWinning = ({ item }: { item: WinningRecord }) => (
		<Pressable
			...
			onPress={() => router.push(`/(modal)/winning-detail?id=${item.id}`)}
		>
			{/* 头部信息 */}
