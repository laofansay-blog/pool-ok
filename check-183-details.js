...
async function check183Details() {
    try {
        console.log('查询第193期的详细数据...')
        ...
        bets.forEach((bet, index) => {
            ...
            if (typeof bet.selected_numbers === 'object' && !Array.isArray(bet.selected_numbers)) {
                ...
                for (let group = 1; group <= 10; group++) {
                    ...
                    if (groupNumbers.length > 0) {
                        ...
                        if (isWinning) {
                            winningGroups++
                            // 计算这组中有多少注中奖
                            winningBets += groupNumbers.filter(num => num === winningNumber).length
                        }
                ...
                if (bet.metadata && bet.metadata.original_bets) {
                    ...
                    console.log(`应得赔付: ${totalWinningBets} × 2 × 9.8 = ${totalWinningBets * 2 * 9.8}元`)
                    console.log(`数据库实际赔付: ${bet.actual_payout}元`)
                    console.log(`差异: ${bet.actual_payout - (totalWinningBets * 2 * 9.8)}元`)

                    if (bet.actual_payout !== totalWinningBets * 2 * 9.8) {
                        console.log('⚠️ 赔付金额不匹配！需要重新计算')
                    }
                } else {
                    console.log('⚠️ 没有原始投注明细数据')
                }
            }
